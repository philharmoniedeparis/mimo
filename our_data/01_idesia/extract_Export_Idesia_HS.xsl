...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:lido="http://www.lido-schema.org" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs"
    ...
    <xsl:template match="//Idesia/term">
        ...
        <xsl:choose>
            <xsl:when test="$nom=''"/>
            <xsl:when test="not (starts-with($nom,'1') or starts-with($nom,'2') or starts-with($nom,'3') or starts-with($nom,'4') or starts-with($nom,'5'))"/>
            <xsl:otherwise>
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:element name="term">
                            ...
                            <xsl:element name="mimoType">hs</xsl:element>
